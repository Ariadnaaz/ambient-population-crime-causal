...
def load_city_data(city_folder, crime_types):
    ...
    df_footfall = make_datetime_index_3h(df_footfall)
    df_crimes = make_datetime_index_3h(df_crimes)

    return df_footfall, df_crimes, original_hex_order


def make_datetime_index_3h(df):
    ...
    df["datetime"] = pd.to_datetime(df["year"].astype(str) + "-01-01") \
     ...
     # floor to 3-hour bins
    df["datetime"] = df["datetime"].dt.floor("3H")

    return (df.drop(columns=["time_key", "hour_of_year", "year"]).groupby("datetime", as_index=True).sum())


def expand_daily_to_3h(df_cov):
...
def load_covariates(city):
    ...
    return expand_daily_to_3h(df_sports), expand_daily_to_3h(df_holidays), expand_daily_to_3h(df_extras)
...
def generate_regression_results_model5(lags, cities, model_name, crime_types, covariates, footfall_factor,n_jobs=-1, B=500):
    ...
    for city in cities:
        ...
        for crime in crime_types:
            ...
            # sprt to use the categorical order, not alphabetical
            ...
            fe_str = " + ".join(["hex_id", "three_hour_block", "day_of_week", "month", "year"])
            ...
            city_results_df.to_csv(
                f"results/Model_with_2SRI/{city}_Pfixest_{model_name}_{crime_str}_3H_N{n_jobs}_B{B}_FF{footfall_factor}.csv",
                index=False
            )
