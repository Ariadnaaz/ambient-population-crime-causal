...
def load_city_data(city_folder, crime_types):
    ...
    df_footfall = make_datetime_index_12h(df_footfall)
    df_crimes = make_datetime_index_12h(df_crimes)

    return df_footfall, df_crimes, original_hex_order


def make_datetime_index_12h(df):
    ...
    df["datetime"] = df["datetime"].dt.floor("12H")

    return (df.drop(columns=["time_key", "hour_of_year", "year"]).groupby("datetime", as_index=True).sum())


def expand_daily_to_12h(df_cov):
    df_cov = df_cov.copy().astype(int)
    df_cov.index = pd.to_datetime(df_cov.index)

    return df_cov.reindex(df_cov.index.repeat(2)).assign( # duplicates each daily row 2 times (once per 12h block)
            datetime=lambda x: np.repeat(df_cov.index, 2)+ pd.to_timedelta(np.tile([0, 12], len(df_cov)), unit="h")
        ).set_index("datetime")
...
def load_covariates(city):
    ...
    return expand_daily_to_12h(df_sports), expand_daily_to_12h(df_holidays), expand_daily_to_12h(df_extras)
...
def generate_regression_results_model5(lags, cities, model_name, crime_types, covariates, footfall_factor,n_jobs=-1, B=500):
    ...
    for city in cities:
        ...
        for crime in crime_types:
            ...
            fe_str = " + ".join(["hex_id", "twelve_hour_block", "day_of_week", "month", "year"])

            exog_covs = ["footfall_lag1", "W_footfall_lag1"] + covariates
            instruments = ["footfall_lag2", "footfall_lag3"]
            W_instruments = ["W_footfall_lag2", "W_footfall_lag3"]
