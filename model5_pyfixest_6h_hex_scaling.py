...
def single_bootstrap_iteration(
    ...
    try:
        ...
        del m1_lag0, m1_Wlag0, m1_Wcrime
        gc.collect()
...
def full_2stage_cluster_bootstrap(
    ...
    formula2,
    original_coefs,
    B, footfall_factor, seed=42, n_jobs=-1
    ):
    ...
    boot_se = boot_df.std(ddof=1) # sample standard deviation, since default 0 is population std
    ...
    return boot_se, boot_p, boot_p_v2, ci
...
def generate_regression_results_model5(lags, cities, model_name, crime_types, covariates, footfall_factor, n_jobs=-1, B=500):
    print("Results using footfall factor:", footfall_factor)
    for city in cities:
        print(f"\n=== CITY: {city} ===")
        ...
        for crime in crime_types:
            ...
            # sample correction
            hex_scaling = pd.read_csv(
                f"./Normalization_hex/{city}_hexagon_level_scaling_factors.csv",
                usecols=["hex_index", "scaling_factor"]
            )
            # computes the average from the valid hexagons and fills in the inf ones (cases when pop16+ is 0 for privacy reasons)
            city_avg = hex_scaling.loc[hex_scaling["scaling_factor"] != np.inf, "scaling_factor"].mean()
            hex_scaling["scaling_factor"] = hex_scaling["scaling_factor"].replace(np.inf, city_avg)

            df = df.merge(hex_scaling, left_on="hex_id", right_on="hex_index", how="left")
            df["footfall"] = df["footfall"] * df["scaling_factor"]
            print(df["scaling_factor"].isna().sum(), "missing scaling factors after merge - should be 0")
            df.drop(columns=["hex_index", "scaling_factor"], inplace=True)
            ...
            # lags & spatial lags (add two more lags for instruments)
            ...
            exog_covs = ["footfall_lag1", "footfall_lag2", "W_footfall_lag1", "W_footfall_lag2"] + covariates
            ...
            print("\n" + "=" * 50)
            print("INSTRUMENT STRENGTH DIAGNOSTICS (First-Stage F-tests)")
            print("=" * 50)

            # get coefficient names to find the right indices
            coef_names_lag0 = list(model1_lag0.coef().index)
            coef_names_Wlag0 = list(model1_Wlag0.coef().index)
            coef_names_Wcrime = list(model1_WCrime.coef().index)

            def build_R_matrix(coef_names, vars_to_test):
                """Build restriction matrix for Wald test: coefficients in vars_to_test = 0"""
                R = np.zeros((len(vars_to_test), len(coef_names)))
                for i, var in enumerate(vars_to_test):
                    R[i, coef_names.index(var)] = 1
                return R

            R_lag0 = build_R_matrix(coef_names_lag0, ["footfall_lag3", "footfall_lag4"])
            R_Wlag0 = build_R_matrix(coef_names_Wlag0, ["W_footfall_lag3", "W_footfall_lag4"])
            R_Wcrime = build_R_matrix(coef_names_Wcrime, ["W_crime_lag2", "W_crime_lag3"])

            # run Wald tests (q defaults to zeros, which is what we want)
            f_results = {}
            ...
            for endog_var, result in f_results.items():
                ...
                print(f"{endog_var:20s} | F = {f_stat:8.2f} | p = {p_val:.6f} | {status}")

            print("=" * 50)
            print("Rule of thumb: F < 10 suggests weak instruments (Stock & Yogo)")
            print("=" * 50 + "\n")
            ...
            del model1_lag0, model1_Wlag0, model1_WCrime
            gc.collect()
            ...
            boot_se, boot_p, boot_p_v2, ci = full_2stage_cluster_bootstrap(
                df=df,
            ...
            city_results_df.to_csv(
                f"results/Model_with_2SRI/{city}_Pfixest_{model_name}_{crime_str}_6H_N{n_jobs}_B{B}_FF{footfall_factor}.csv",
                index=False
            )
...
if __name__ == "__main__":
    ...
    generate_regression_results_model5(
        lags=[0, 1, 2],
        cities=['Philadelphia'],
        model_name='bootstrap_parallel_hex_scaling',
        crime_types = [
                        ["Burglary"],
                        ["Motor Vehicle Theft"],
                        ["Assault"],
